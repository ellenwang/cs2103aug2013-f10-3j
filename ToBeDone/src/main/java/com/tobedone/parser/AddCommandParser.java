...
public class AddCommandParser extends CommandParser {
	...
	public Command parse(String paraString) throws Exception{
			...
			if(indexoffrom!= Constants.NOT_FOUND_INDEX 
				...
				String startTimeString = paraString.substring(indexoffrom+5,indexofto-Constants.ONE_LOOKAHEAD);
				startTime = parseDate(startTimeString);
				
				if(indexofpriority != Constants.NOT_FOUND_INDEX){
					endTimeString = paraString.substring(indexofto+3,indexofpriority-1);
				}else{
		...
		if (indexofpriority != Constants.NOT_FOUND_INDEX) {
			...
			return new AddCommand(description,startTime,endTime,deadline,priority);		
	}
