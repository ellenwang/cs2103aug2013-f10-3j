 ...
 * @since 01-09-2013
 * 
 *        This class will generate an update command by parsing input command
 *        string It will throws exception when the index is wrong
 */
public class UpdateCommandParser extends CommandParser {
	...
	public Command parse(String paraString) throws Exception {
		...
		if (beginby == maxindex && byString != null
		...
		else if (endfromto > maxindex && fromtoString != null
		...
		else if (beginfrom == maxindex && fromString != null
		...
		else if (beginto == maxindex && toString != null
				&& paraString.endsWith(toString) && beginto > endfromto) {
	...
	protected Date getNewstartTime() {
		return newstartTime;
	}

	protected Date getNewendTime() {
		return newendTime;
	}

	protected Date getNewdeadline() {
		return newdeadline;
	}
