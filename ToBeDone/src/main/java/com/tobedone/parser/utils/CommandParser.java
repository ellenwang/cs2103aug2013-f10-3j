...
public class CommandParser {
	...
	// @author A0117215R
	// this method will be overriding by it subclasses
	...
	// @author A0117215R
	/**
	 * parse the task priority from parameters string 
	 * 
	 * @return the priority of type int
	 */
	...
	public CommandParser getCommandParser(String type) throws Exception {
		...
		try {
			...
			throw new CommandNotFoundException(
					Constants.MSG_ERROR_INVALID_COMMAND);
		}
	...
	// @author A0117215R
	/**
	 * parse the task index from parameters string 
	 * 
	 * @return the index of type int
	 */
	...
	// @author A0117215R
	/**
	 * parse the time string into the one follow standard pattern HH:mm
	 * 
	 * @return the time string follow the standard format HH:mm
	 */
	...
	private String parseDay(String datePattern) {
		if (datePattern.matches(Constants.REGEX_SHORT_DAY)) {
			...
			datePattern = futureday + Constants.HYPHEN + currentMonth;
		}
	...
	/**
	 * if today is Wed, when user input "this mon", it will be mon this week if
	 * user just input mon or "next mon", it will be mon next week however if
	 * user input "this fri" or just "fri", it will be fri week only "next fri"
	 * will be next fri.
	 * 
	 * @return the command name
	 */
