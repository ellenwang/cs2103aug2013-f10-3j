 ...
 * handled and tested by the Command classes. So just check the functionalities.
 * 
 * @author A0105682H
 * 
 */
public class ToDoListAtd {
	...
	public void completeTaskTest() throws IOException, TaskNotExistException,
			ParseException {
		toDoList.completeTask(2);
		result = toDoList.getMatchingTasks();
		expected.get(2).setStatus(TaskItem.Status.FINISHED);
		assertTrue(this.compareVector(result, expected));

	}

	// Tests the updateTask() method
	public void clearTasksTest() throws ParseException, TaskNotExistException,
			IOException {
		toDoList.clear();
		result = toDoList.getMatchingTasks();
		expected.clear();;
	 ...
	 * 
	 * @param result
	 *            The result vector list of TaskItem after the execution of the
	 *            command.
	 * @param expected
	 *            The expected vector list of the TaskItem.
	 * @return Return true if two vectors are equal and false, otherwise.
	 */
