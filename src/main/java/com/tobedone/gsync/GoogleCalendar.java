...
public class GoogleCalendar {
	...
	public Vector<TaskItem> updateGcal(String calendarId, Vector<TaskItem>allTasks) {
		Vector<TaskItem>updatedItems = new Vector<TaskItem>();
		...
		try{
			...
			do {
				...
				for (Event event : items) {
					...
					for(TaskItem task : allTasks){
						if(task.getDescription().equals(event.getSummary())){
							client.events().delete(Constants.DEAFAULT_CALENDAR, event.getId()).execute();
							taskToUpdate = task;
							break;
						}
					}
					if(taskToUpdate != null){
						...
						updatedItems.add(taskToUpdate);	
						tasksFromGoogle.add(taskToUpdate);
					}else{
						tasksFromGoogle.add(gParser.eventToTask(event));
						client.events().delete(calendarId, event.getId());  //delete or not?
					}
				}
				pageToken = events.getNextPageToken();
			} while (pageToken != null);
